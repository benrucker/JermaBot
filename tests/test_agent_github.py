...
These tests stub gh at run_authed, the one place the workspace shells out
to it. The matching itself is the real thing: the agent's branch prefix,
the pull request footer, and the starter prompt quoted in the first
commit.

The listing does not ask gh for commits. GitHub prices that per pull
request listed and rejects the query well below the hundred the search asks
for, so the search fetches a candidate's commits one at a time, and only
for the candidates that need them.
"""
import json

import pytest

from cogs.utils.agent_config import AGENT_PR_FOOTER, AgentRepo
...
def workspace(tmp_path):
    """A workspace with one repo and a stubbed gh.

    `listed` answers `gh pr list`, `commits` answers `gh pr view` (keyed
    by url), and the workspace records every argv it was called with."""
...
def viewed(workspace) -> list[str]:
    """The pull requests the search fetched commits for, one at a time."""
    return [argv[3] for argv in workspace.calls if argv[2] == 'view']


async def test_a_pull_request_matches_by_its_first_commit(workspace):
    workspace.listed = [pull_request()]
    workspace.commits = {URL: commits_quoting(STARTER)}

    found = await workspace.find_conversation_on_github(STARTER)

    assert found == (BRANCH, {'demo': URL})
    assert viewed(workspace) == [URL]


async def test_another_conversations_pull_request_does_not_match(workspace):
    workspace.listed = [pull_request()]
    workspace.commits = {URL: commits_quoting('some other request')}

    assert await workspace.find_conversation_on_github(STARTER) is None


async def test_an_empty_request_matches_nothing(workspace):
    """A thread whose starter is all mention has nothing to quote, so the
    search does not spend a commit query on it."""
    workspace.listed = [pull_request()]
    workspace.commits = {URL: commits_quoting('')}

    assert await workspace.find_conversation_on_github('') is None
...
async def test_a_branch_without_the_agent_prefix_is_not_ours(workspace):
    workspace.listed = [pull_request(head='feature/something')]
    workspace.commits = {URL: commits_quoting(STARTER)}
...
async def test_the_branch_comes_from_the_newest_repos_pull_request(tmp_path):
    ...
    async def stubbed(argv, cwd, stdin_data=None):
        if argv[2] == 'view':
            return json.dumps({'commits': commits_quoting(STARTER)})
...
async def test_commits_are_fetched_only_where_they_could_decide_it(workspace):
    """One `gh pr view` per candidate is the price of the matching rule,
    so the search pays it only for the agent's own pull requests, and
    stops at the first that quotes the request."""
    footerless = 'https://github.com/local/demo/pull/4'
    old = 'https://github.com/local/demo/pull/5'
    older = 'https://github.com/local/demo/pull/2'
    workspace.listed = [
        ...
        pull_request(url=older, created='2026-08-01T00:00:00Z'),
    ]
    workspace.commits = {old: commits_quoting('some other request'),
                         URL: commits_quoting(STARTER),
                         older: commits_quoting(STARTER)}
    ...
    assert viewed(workspace) == [old, URL]
